...
class EduDataset(Dataset):
    def __init__(self, root_folder, metadata_file):
        ...
        """
        Dataset.__init__(self, root_folder, metadata_file, "Edu's dataset built for face detection.")
    ...
    def get_formatted_metadata(self):
        ...
        for metadata_line in self.metadata_content:
            ...
            file_id = values[0].replace("'", "").strip()

            has_faces_flag = bool(int(values[1]))

            metadata_element = [has_faces_flag]
